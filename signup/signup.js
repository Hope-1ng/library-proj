function validate() {
  ...
  let three = document.getElementById("three-3");
  validatePhone(pnumber);
  validateMail(email);
  validatePass(password);
  matchfix(three, password);
...
function matchfix(three, password) {
  if (three.value === "") {
    document.getElementById("three").innerHTML =
      '<i class="bi-exclamation-circle  "><p style="color:red;display:inline;">pls re-enter the password </p></i>';
    three.style.border = "red 3px solid";
    return false;
  }
  if (three.value !== password.value) {
    document.getElementById("three").innerHTML =
      '<i class="bi-exclamation-circle  "><p style="color:red;display:inline;"> not a match </p></i>';
    three.style.border = "red 3px solid";
    return false;
  } else {
    return true;
...
function validatePass(password) {
  ...
  if (password.value.length < 8) {
    ...
    document.getElementById("exi-2").innerHTML =
      '<i class="bi-exclamation-circle text-warning"><p style="color:orange; display:inline;">week</p></i>';
    password.style.border = "orange  3px solid";
    return false;
  } else if (password.value.length >= 10) {
    document.getElementById("exi-2").innerHTML =
      '<i class="bi-hand-thumbs-up-fill  text-success "><p style="color:green; display:inline;">power!!</p></i>';
